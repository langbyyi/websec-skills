...
> **AI LOAD INSTRUCTION**: Expert authentication bypass techniques. Covers SQL injection-based login bypass, password reset flaws, token predictability, account enumeration, brute force bypass, and multi-factor auth bypass. Distinct from JWT/OAuth (covered in ../jwt-attacks/SKILL.md). Focus on the login mechanism itself.
...
| Signal | Probe | Why |
|---|---|---|
| Login form found | Default creds `admin:admin` | Vendor defaults still dominate internal panels |
| Login rejects defaults | `' OR '1'='1'--` in username | Classic SQLi login bypass |
| SQLi fails / password reset flow | Predictable token, Host header injection | Reset flaws bypass auth entirely |
| MFA step after login | Skip 2FA page directly with session cookie | Session often set before MFA check |
| Rate limiting on login | `X-Forwarded-For: N.N.N.N` rotation | IP-based lockout bypass |
| Captcha on login | Remove captcha param, empty value, method swap | Captcha enforcement often client-side only |

```bash
# Quick test — SQLi login bypass
curl -s -D- 'https://target/login' -d "username=admin'--&password=x"
```
...
After reducing entry points, default credentials, username variants, port focusing, and dictionary size selection are consolidated here.
...
Priority ports and services: 80/443/8080/8443 admin panels, 22 SSH, 21 FTP, 3306/5432/6379/27017 data or management services.
...
Login form found?
├── Default credentials?
│   ├── Try vendor defaults (admin:admin, root:root, tomcat:tomcat)
│   └── No default access → proceed to next checks
├── Brute force possible?
│   ├── No rate limit → credential stuffing / password spraying
│   ├── IP-based lockout → bypass with X-Forwarded-For rotation
│   ├── Account lockout → username cycling (one pass, many users)
│   └── Robust rate limiting → move to bypass techniques
├── Login bypass techniques?
│   ├── SQL injection on login fields (' OR 1=1--)
│   ├── Null byte in username (admin%00)
│   ├── Unicode normalization (homoglyphs)
│   └── Long password truncation (bcrypt 72-byte)
├── MFA bypass?
│   ├── Session cookie set before 2FA → skip 2FA page directly
│   ├── No rate limit on 2FA codes → brute force TOTP
│   ├── 2FA code reuse → replay previous code
│   └── 2FA not enforced on all endpoints → direct API access
└── Password reset flaws?
    ├── Predictable token (timestamp, MD5 of email, sequential)
    ├── Token not expiring or reusable
    ├── Host header injection → token sent to attacker
    ├── Token leaked in Referer header
    └── Step skip → jump to "set new password" without token validation
```
...
## 13. INSECURE RANDOMNESS — TOKEN PREDICTION
...
| Tool | Use Case |
|------|----------|
| `http_framework_test` | Send crafted login, password reset, and authentication requests to test bypass logic |
| `http_repeater` | Replay and modify authentication requests to test token predictability and step skipping |
| `hydra_attack` | Brute force login forms, SSH, FTP, and other services with credential lists |
| `browser_agent_inspect` | Browser inspection to observe authentication flows, session cookies, and 2FA behavior |
| `nuclei_scan` | Automated scanning for known authentication bypass vulnerabilities and default credentials |

## RELATED ROUTING

- [csrf](../csrf/SKILL.md) — CSRF often chains with auth flaws for account takeover
- [oauth-oidc](../oauth-oidc/SKILL.md) — OAuth/OIDC misconfigurations cause auth bypass
- [jwt-oauth](../jwt-attacks/SKILL.md) — JWT claim abuse and token forgery bypass auth
- [business-logic](../business-logic/SKILL.md) — 2FA skip, password reset flow bypass
- [recon](../recon-and-methodology/SKILL.md) — Default credential and exposed auth endpoint discovery
