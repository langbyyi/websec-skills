...
| Signal | Probe | Why |
|---|---|---|
| CDN/cache headers (X-Cache, Age, Via) | Append `.css` to authenticated endpoint URL | Path confusion → cached auth data |
| Authenticated JSON endpoint | Add `/nonexistent.js` to path | Cache stores sensitive response |
| Cache-Control: private? | Test with `X-Forwarded-Host: evil.com` | Unkeyed header → cache poisoning |
| Vary header missing? | Send request with different Accept-Encoding | Cache key incomplete |
| Query string in cache key? | Add `?cachebuster=random` | Check if query params are keyed |

```bash
# Quick test: does appending .css cache the response?
curl -H "Cookie: session=YOURSESSION" https://target.com/api/user/profile.css
curl https://target.com/api/user/profile.css  # without cookies - cached?
```
...
```
Authenticated endpoint with sensitive data?
├── Cache layer present (CDN, reverse proxy)?
│   ├── No cache layer → no cache deception risk
│   └── Cache layer detected (X-Cache, Age, Via headers)
├── Cache-Control headers on authenticated endpoints?
│   ├── no-store, private → properly protected
│   └── Missing or public → test path confusion
├── Path confusion possible?
│   ├── Append .css/.js/.png to API path → still serves auth data?
│   │   ├── Yes → cacheable extension + auth data = deception
│   │   └── No → try path separators (;.css, %2F.css, /.css)
│   └── Path normalization differs between CDN and app?
│       └── Yes → exploit mismatch
├── Extension trick cached?
│   ├── X-Cache: HIT on second request → confirmed cached
│   └── X-Cache: MISS → not cached, try different extension
└── Cache poisoning angle?
    ├── Unkeyed headers reflected (X-Forwarded-Host)?
    │   └── Reflected in response + cached → poisoning
    └── All inputs keyed → poisoning not viable
```
...
| Tool | Use Case |
|------|----------|
| `http_framework_test` | Send crafted requests with cacheable extensions and unkeyed headers to test path confusion |
| `http_repeater` | Replay requests to verify cache HIT/MISS behavior and poisoning persistence |
| `browser_agent_inspect` | Browser inspection to observe cached responses and CDN behavior from user perspective |
| `burpsuite_alternative_scan` | Comprehensive web scanning to identify cache layers and test deception/poisoning |

## RELATED ROUTING

- [cors](../cors/SKILL.md) — cross-origin misconfigurations complement cache deception for data theft
- [request-smuggling](../request-smuggling/SKILL.md) — HTTP smuggling can poison cache entries via desync between proxy and cache
- [csrf](../csrf/SKILL.md) — cross-site attacks combine with cache poisoning to serve malicious content
