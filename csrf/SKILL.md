...
- [cors cross origin misconfiguration](../cors/SKILL.md) when JSON endpoints become readable cross-origin
- [oauth oidc misconfiguration](../oauth-oidc/SKILL.md) when login, account linking, or callback binding relies on OAuth state
...
| Signal | Probe | Why |
|---|---|---|
| State-changing form? | Remove CSRF token, submit | Token not validated = trivial bypass (most common finding) |
| Token removed, still works? | Change token to random value, submit | Confirms server ignores token entirely |
| Method accepts GET? | Convert POST to GET with same params | SameSite=Lax allows GET cookies cross-site |
| SameSite cookie attribute? | Check Set-Cookie: SameSite=Lax / None / Strict | Lax blocks POST but allows GET; None = classic CSRF applies |
| Referer/Origin validated? | Send request with empty Referer header | Some checks only validate presence, not value |

```bash
# Quick test: remove token and replay state-changing request
# In Burp: delete csrf_token param → Forward
# If 200 OK → trivial CSRF
```
...
```
State-changing request (password change, transfer, admin action)?
├── CSRF token present?
│   ├── No token → trivially exploitable
│   ├── Token not validated (remove it) → trivial bypass
│   ├── Token tied to wrong session → use another user's token
│   ├── Static / predictable token → forge token
│   └── Token properly validated → check cookie-based patterns
├── SameSite cookie attribute?
│   ├── SameSite=Strict → block cross-site (look for subdomain XSS)
│   ├── SameSite=Lax → bypass via GET method or 2-minute window
│   └── SameSite=None or missing → classic CSRF applies
├── Referer/Origin check?
│   ├── No check → exploitable
│   ├── Present but bypassable → spoof subdomain or remove header
│   └── Properly validated → move to content-type checks
├── JSON content-type required?
│   ├── text/plain accepted → use text/plain form enctype trick
│   ├── CORS allows credentials → XHR/fetch JSON CSRF
│   └── Strict JSON only → check multipart fallback
└── OAuth state parameter missing?
    └── OAuth CSRF → bind victim to attacker's provider
```

## 11. CSRF TESTING CHECKLIST
...
| Tool | Use Case |
|------|----------|
| `http_framework_test` | Craft custom HTTP requests to test CSRF token bypass and state-changing actions |
| `http_repeater` | Replay and modify requests to test token validation and SameSite cookie behavior |
| `browser_agent_inspect` | Inspect pages for SameSite cookie attributes, CORS headers, and CSRF token presence |
| `burpsuite_alternative_scan` | Comprehensive web scanning to discover missing CSRF protections |

## 14. MULTIPART CSRF & CLIENT-SIDE PATH TRAVERSAL
