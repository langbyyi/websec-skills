...
Routing hint: If the task comes from supply chain / repository exposure / CI build reconnaissance, first cross-reference `recon-for-sec` to list internal package names and check alignment with public registries.
...
Routing hint: After package name enumeration, consider PoC only in authorized environments; public registry queries are typically passive reconnaissance.
...
- [ ] Identify all package managers in use (npm, pip, RubyGems, Maven, Composer, Docker) from manifest files or CI configs
- [ ] Extract internal/private package names from committed manifests: `package.json`, `requirements.txt`, `Gemfile`, `pom.xml`, `composer.json`
- [ ] Check if internal package names are claimable/squattable on public registries (`npm view`, `pip install --dry-run`, `gem search --remote`, Maven Central search)
- [ ] Test registry priority: verify which registry (private vs public) wins when both serve the same package name
- [ ] Test version resolution: publish a higher semver on public registry within the target's declared range and confirm it is selected
- [ ] Test typosquatting potential: check for common misspelling variants of internal package names on public registries
- [ ] Verify lockfile enforcement: check if `package-lock.json`, `poetry.lock`, `Gemfile.lock`, `composer.lock` are present, up-to-date, and enforced in CI
- [ ] Test `.npmrc` / `.pypirc` / CI config misconfiguration: multiple registries, missing scope locks, `--extra-index-url` usage
- [ ] Validate with callback PoC (authorized only): publish controlled package with DNS/HTTP callback to prove code execution
- [ ] Verify defensive controls: scoped packages with org-owned scopes, single private index, canonical repository settings, lifecycle script blocking

## MCP TOOLS

| Tool | Use Case |
|------|----------|
| `http_framework_test` | Send crafted requests to test package registry resolution and manifest exposure |
| `http_repeater` | Replay and modify requests to test private package name leaks via APIs and error responses |
| `nuclei_scan` | Automated scanning for exposed manifests, .npmrc, requirements.txt, and config files |
| `api_fuzzer` | Fuzz package registry APIs to discover version resolution behavior and index precedence |
