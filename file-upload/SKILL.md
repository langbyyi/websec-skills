...
- [path traversal lfi](../path-traversal/SKILL.md) when filename, extraction path, or include path becomes file-system control
- [xss cross site scripting](../xss/SKILL.md) when uploads are rendered in browser contexts
- [xxe xml external entity](../xxe/SKILL.md) when SVG, OOXML, or XML imports are accepted
- [cmdi command injection](../command-injection/SKILL.md) when a processor, converter, or media pipeline executes system tools
- [business logic vulnerabilities](../business-logic/SKILL.md) when quotas, overwrite rules, approvals, or storage paths create logic bugs
...
| Signal | Probe | Why |
|---|---|---|
| Upload .php blocked? | Try `.phtml`, `.php5`, `.phar` | Extension blacklist incomplete |
| Change Content-Type? | Set `Content-Type: image/jpeg` with PHP body | Server trusts MIME over extension |
| Double extension? | Upload `shell.php.jpg` or `shell.asp;.jpg` | Parser quirks: Apache multi-ext, IIS semicolon |
| Null byte bypass? | Upload `shell.php%00.jpg` | Legacy PHP/JVM truncates at null byte |
| SVG accepted? | Upload SVG with `<script>` or XXE inside | SVG = active content / XXE vector |

```bash
# Quick upload bypass sequence
# 1. Upload shell.php          → blocked? proceed
# 2. Upload shell.phtml        → blocked? proceed
# 3. Change Content-Type only  → accepted? check if rendered as code
# 4. Try shell.php.jpg         → accepted? check server parsing quirk
# 5. Try null byte shell.php%00.jpg → legacy truncation bypass
```
...
This small sample already covers the main use cases of the former standalone upload payload helper; no additional entry point is needed for first-round selection.
...
| Observation | Pivot |
|---|---|
| SVG or XML accepted | [xxe xml external entity](../xxe/SKILL.md) |
| filename or metadata reflected | [xss cross site scripting](../xss/SKILL.md) |
| converter or processor shells out | [cmdi command injection](../command-injection/SKILL.md) |
| extraction path looks controllable | [path traversal lfi](../path-traversal/SKILL.md) |
| overwrite, quota, approval, or tenant bug | [business logic vulnerabilities](../business-logic/SKILL.md) |
...
│   ├── Blacklist → double extension (shell.php.jpg), case toggles (PhP), trailing dot
│   ├── Whitelist → alternate extensions (.phtml, .php5, .shtml)
│   └── No extension check → upload web shell directly
├── Content-Type validation?
│   ├── Mismatch multipart Content-Type → image/jpeg with PHP body
│   └── No content-type check → any MIME with dangerous extension
├── Magic bytes / file content check?
│   ├── Polyglot file → valid header + dangerous payload appended
│   └── Image-only policy → SVG with embedded JS/XXE, EXIF metadata injection
├── Double extension parsing?
│   ├── IIS: shell.asp;.jpg (semicolon truncation) or ::$DATA (ADS)
│   ├── Apache: shell.php.jpg (multi-extension, AddHandler)
│   └── Apache CVE-2017-15715: shell.php\x0a (newline bypass)
├── Null byte in filename?
│   └── file.php%00.jpg → extension truncation (legacy)
├── Path traversal in filename?
│   └── ../../../var/www/html/shell.php → write outside upload directory
├── Server parsing quirk?
│   ├── Nginx cgi.fix_pathinfo=1 → /uploads/image.jpg/anything.php executes as PHP
│   └── WebLogic / Tomcat PUT method → direct JSP upload
├── Archive or document import?
│   ├── Zip slip → path traversal in archive member names
│   ├── OOXML import → XXE in embedded XML
│   └── CSV import → formula injection in analyst workflows
├── Processing chain after upload?
│   ├── Image thumbnailing → ImageMagick/library bugs
│   ├── HTML-to-PDF → SSRF/script execution
│   └── AV/DLP scanner → race condition or nested content bypass
└── Authorization / storage issue?
    ├── Predictable file path → cross-tenant access
    ├── Overwrite other user's file → content replacement
    └── A/B test: upload as UserA, access as UserB → IDOR on files
```

## TESTING CHECKLIST

- [ ] Test extension bypass: double extension (`shell.php.jpg`), case toggles (`PhP`), trailing dot (`file.php.`)
- [ ] Test content-type spoofing: mismatch `Content-Type` header in multipart upload
- [ ] Test magic bytes: polyglot files with valid header and dangerous payload appended
- [ ] Test double extension parsing: IIS semicolon (`shell.asp;.jpg`), Apache multi-extension (`shell.php.jpg`)
- [ ] Test null byte in filename: `file.php%00.jpg` for extension truncation
- [ ] Test path traversal in filename: `../../../var/www/html/shell.php` in upload path
- [ ] Test client-side bypass: intercept and modify validation responses, disabled JavaScript
- [ ] Test SVG upload for stored XSS and XXE
- [ ] Test image metadata injection: EXIF fields with XSS payloads reflected in galleries
- [ ] Test server parsing quirks: Nginx `cgi.fix_pathinfo`, Apache CVE-2017-15715 newline, IIS `::$DATA`
- [ ] Test archive extraction: zip slip, nested path names, decompression bombs
- [ ] Test A/B authorization: upload as UserA, attempt read/replace/delete as UserB
- [ ] Verify post-upload retrieval: inline render vs forced download, predictable URLs, public/private boundaries

## MCP TOOLS

| Tool | Use Case |
|------|----------|
| `http_framework_test` | Send crafted multipart upload requests with validation bypass payloads |
| `http_repeater` | Replay and modify upload requests to test extension and content-type bypass |
| `nuclei_scan` | Automated scanning for known file upload misconfigurations and CVEs |
| `burpsuite_alternative_scan` | Comprehensive web scanning to discover upload endpoints and test validation |
| `browser_agent_inspect` | Browser inspection to verify upload behavior, preview rendering, and access controls |