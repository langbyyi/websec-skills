...
| Signal | Probe | Why |
|---|---|---|
| Object reference in URL/body/header/cookie | Change ID to another user's | Direct BOLA test |
| Changed ID returns same response | Access another user's data as them | IDOR confirmed |
| Sequential integer IDs | Iterate `id=N+1`, `id=N-1` | Easy prediction, high hit rate |
| UUID IDs | Collect UUIDs from own account responses | GUIDs leak via other API endpoints |
| Extra fields accepted in POST/PUT | Add `"role":"admin"` or `"isAdmin":true` | Mass assignment → privilege escalation |
| Admin endpoint accessible as regular user | `GET /api/admin/users` with low-priv token | BFLA confirmed |

```bash
# Quick test — swap user ID in API path
curl -s -H "Authorization: Bearer $TOKEN" 'https://target/api/users/VICTIM_ID/profile'
```
...
```
Object reference found in request?
├── Sequential integer ID (id=1001)?
│   ├── Increment/decrement → access other users' data (BOLA)
│   └── Mass assignment via extra JSON fields → role escalation
├── UUID/GUID ID?
│   ├── UUID v1 (time-based) → predictable → enumerate
│   ├── UUID v4 → collect from own account responses, API leaks
│   └── Encoded/hashed ID → decode (base64) or reverse (md5 of int)
├── ID in unusual location?
│   ├── Request body / JSON fields / headers (X-User-ID) / cookies
│   ├── GraphQL arguments → nested object traversal
│   └── WebSocket messages → channel/resource ID swap
├── Different HTTP method on same endpoint?
│   ├── GET blocked but PUT/PATCH/DELETE not checked → modify/delete others' data
│   └── POST to admin endpoint as regular user → BFLA
├── Sub-resource or nested object?
│   └── Access attachment/comment directly without parent auth check → indirect IDOR
├── Mass assignment in POST/PUT body?
│   └── Inject hidden fields (role, isAdmin, verified) → privilege escalation
├── Parameter pollution / type confusion?
│   ├── Array params (id[]=1234&id[]=5678) → auth bypass
│   └── String vs int type confusion → different ORM code path
└── State machine in resource?
    └── Skip workflow states (pending→delivered) → business logic abuse
```

## TESTING CHECKLIST

- [ ] Identify all object references in requests: URL path, query params, JSON body, headers, cookies, GraphQL args
- [ ] Create two test accounts (UserA and UserB) for A-B authorization testing
- [ ] Test sequential IDs: increment/decrement numeric IDs (`id=1001` to `id=1002`)
- [ ] Test UUID predictability: check for v1 time-based UUIDs, collect UUIDs from own account data
- [ ] Test API authorization: replay UserA requests with UserB session token
- [ ] Test all HTTP methods: GET, POST, PUT, PATCH, DELETE on each object endpoint
- [ ] Test mass assignment: inject hidden fields (`role`, `isAdmin`, `verified`, `creditBalance`) in POST/PUT body
- [ ] Test indirect IDOR: access sub-resources (attachments, comments) directly without parent authorization
- [ ] Test BFLA: access admin endpoints (`/api/admin/`) as regular user
- [ ] Test parameter pollution and type confusion: arrays, duplicate params, string vs int IDs
- [ ] Test state machine abuse: skip order/workflow states via status field modification
- [ ] Test ORM filter injection: Django `__startswith`, Prisma `include`, Ransack predicates

## MCP TOOLS

| Tool | Use Case |
|------|----------|
| `http_framework_test` | Craft custom HTTP requests to test IDOR by swapping object IDs across locations |
| `http_repeater` | Replay requests with modified IDs (path, query, body, headers) for A-B testing |
| `api_fuzzer` | Fuzz API endpoints for IDOR by iterating over object identifiers and HTTP methods |
| `browser_agent_inspect` | Inspect API responses and JavaScript for leaked object IDs and hidden fields |
| `nuclei_scan` | Run IDOR and broken access control vulnerability templates against target URLs |

## 17. ORM FILTER CHAIN LEAKS
...
- [jwt-oauth](../jwt-attacks/SKILL.md) — Token abuse enables IDOR escalation
- [authbypass](../authentication-bypass/SKILL.md) — Auth bypass complements BOLA testing
- [api-security](../api-security/SKILL.md) — API router for endpoint discovery before IDOR testing
- [graphql](../graphql/SKILL.md) — GraphQL IDOR via nested object traversal
