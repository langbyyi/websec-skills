...
- [oauth oidc misconfiguration](../oauth-oidc/SKILL.md) for redirect URI, state, nonce, PKCE, and account-binding validation
- [cors cross origin misconfiguration](../cors/SKILL.md) when browser-readable APIs or token leakage may exist cross-origin
- [saml sso assertion attacks](../saml-attacks/SKILL.md) when the target uses enterprise SSO outside OAuth/OIDC
...
| Signal | Probe | Why |
|---|---|---|
| JWT captured? | Decode header+payload (base64) | Identify alg, claims, attack surface |
| alg:none accepted? | Set alg to "none", remove signature | Server skips validation = forge any claims |
| Signature actually checked? | Modify a claim (role→admin), keep old sig | If accepted → signature not enforced |
| Claims decodeable? | Check role, userId, isAdmin, exp | Modify high-value claims to escalate |
| kid/jku in header? | Inject kid SQLi / point jku to attacker JWKS | Control which key validates the token |
| HS256 with weak secret? | hashcat -m 16500 JWT rockyou.txt | Recover secret → forge any token |

```bash
# Decode JWT quickly
echo "HEADER_PART" | base64 -d 2>/dev/null; echo
...
JWT token found?
├── alg:none accepted?
│   └── Remove signature, set alg to "none"/"None"/"NONE" → forge any claims
├── HS256 (HMAC) with weak secret?
│   └── Brute force with hashcat/john/rockyou → recover secret → forge tokens
├── RS256 (asymmetric) in use?
│   ├── Public key discoverable (JWKS endpoint, /certs, HTTPS cert)?
│   │   └── Algorithm confusion: RS256→HS256 → sign with public key as HMAC secret
│   └── jku/x5u header accepted without whitelist?
│       └── Point to attacker-controlled JWKS → sign with own key pair
├── kid (Key ID) header present?
│   ├── kid SQL injection → control HMAC key → forge tokens
│   └── kid path traversal → point to known file (/dev/null → empty key)
├── Claim manipulation possible?
│   ├── Modify role/userId/isAdmin in payload
│   ├── Token not validated after decode → forged claims accepted
│   └── exp/nbf not enforced → expired tokens still valid
└── OAuth flow present?
    ├── State parameter missing → CSRF account takeover
    ├── redirect_uri weakly validated → token theft via open redirect
    ├── Implicit flow → token leakage via Referer/fragment
    └── Scope escalation during token exchange → privilege escalation
```
...
| Tool | Use Case |
|------|----------|
| `jwt_analyzer` | Analyze JWT tokens for algorithm confusion, weak secrets, and claim manipulation |
| `http_framework_test` | Craft custom HTTP requests to test token forgery and OAuth flow abuse |
| `http_repeater` | Replay and modify requests to test token validation bypasses |
| `browser_agent_inspect` | Inspect OAuth flows, token leakage in URLs, and browser-based token handling |

## 15. OAUTH TESTING CHECKLIST
