...
| Signal | Probe | Why |
|---|---|---|
| Redirect parameter found (`url=`, `next=`, `redirect=`) | `https://evil.com` as value | Direct redirect — no filter |
| `https://` blocked | `//evil.com` (protocol-relative) | Bypasses scheme check |
| `//` blocked | `/\evil.com` (backslash) | Browser normalizes backslash to forward slash |
| Domain whitelist | `https://trusted.com@evil.com` (userinfo) | Authority confusion |
| Domain suffix check | `http://evil.com/www.target.com` | Django `endswith` bypass |
| URL encoding in filter | `https://%65vil.com`, double-encode | Encoding bypass |

```bash
# Quick test — basic open redirect
curl -s -D- 'https://target/redirect?url=https://evil.com' | grep -i location
```
...
```
Redirect parameter found (url=, next=, redirect=, return=)?
├── URL validation present?
│   ├── No validation → direct redirect to evil.com
│   ├── Checks for "/" prefix → try //evil.com (protocol-relative)
│   └── Validation exists → attempt bypasses
├── Protocol bypass?
│   ├── //evil.com works → protocol-relative redirect
│   ├── https://trusted.com@evil.com → userinfo bypass
│   └── Blocked → try encoding tricks
├── Domain confusion?
│   ├── evil.com?trusted.com → query string confusion
│   ├── evil.com/www.target.com → Django endswith bypass
│   ├── trusted.com.evil.com → subdomain confusion
│   └── Blocked → try path-based tricks
├── Path traversal style?
│   ├── /\evil.com → backslash trick
│   ├── /%2F%2Fevil.com → encoded slashes
│   └── Blocked → try double-redirect chain via trusted site
└── JavaScript sink (DOM-based)?
    ├── window.location = input → javascript:alert(1)
    └── Check for tabnabbing via target="_blank" without rel="noopener"
```
...
| Tool | Use Case |
|------|----------|
| `http_framework_test` | Craft custom HTTP requests to test redirect parameters and filter bypass payloads |
| `http_repeater` | Replay and modify requests to test redirect behavior with encoding variations |
| `browser_agent_inspect` | Inspect pages for client-side redirect sinks (window.location, meta refresh) |
| `nuclei_scan` | Run open redirect vulnerability templates against target URLs |

## 8. TABNABBING (REVERSE TABNABBING)
...
- [ssrf](../ssrf/SKILL.md) — Open redirect chains into SSRF via server-side redirect following
- [xss](../xss/SKILL.md) — DOM-based open redirect shares sinks with XSS (window.location)
- [csrf](../csrf/SKILL.md) — Open redirect bypasses CSRF Referer-based protections
- [oauth-oidc](../oauth-oidc/SKILL.md) — Open redirect on OAuth redirect_uri steals authorization codes
