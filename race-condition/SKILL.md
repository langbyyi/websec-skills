...
| Signal | Probe | Why |
|---|---|---|
| One-time operation (redeem, claim, verify) | Send 10+ concurrent copies of same request | Non-atomic check-then-update → duplicate success |
| Concurrent requests yield duplicate `200`/`201` | Single-packet attack (HTTP/2) or last-byte sync | Tighter timing alignment wins narrow TOCTOU windows |
| Rate limit bypass needed | Burst N requests in parallel vs sequential | Parallel burst may pass counter before increment |
| Multi-step workflow (create → pay → confirm) | Parallel pipeline: confirm on B while pay on A is in-flight | State machine skips or inconsistent final state |
| Balance / quota / stock deduction | Send 20 parallel with amount = full balance | Double-spend if read-then-write not atomic |

```bash
# Quick test — Turbo Intruder gate pattern (conceptual)
# Queue 30 copies of the same request, release gate simultaneously
```
...
**Routing note**: If the scenario leans more toward "business rules / pricing / workflow skips", load `skills/business-logic/SKILL.md`; this file focuses on **concurrency and transport-layer synchronization**.

---

## TESTING CHECKLIST

- [ ] Identify state-changing endpoints (transfer, apply coupon, vote, redeem)
- [ ] Test concurrent requests via Turbo Intruder or burpsuite_alternative_scan
- [ ] Test single-packet attack (HTTP/2) for timing-sensitive races
- [ ] Test TOCTOU in file operations, database reads-writes
- [ ] Test rate limit bypass via concurrent connections
- [ ] Test one-time token reuse (password reset, email verification)
- [ ] Test double-spend on credits, gift cards, loyalty points
- [ ] Test race in multi-step flows (checkout, approval, submission)
- [ ] Verify with timing analysis (compare single vs concurrent response diffs)

## MCP TOOLS

| Tool | Use Case |
|------|----------|
| `http_framework_test` | Send crafted HTTP requests to test concurrent state-changing operations |
| `http_repeater` | Replay and modify requests to reproduce race condition evidence |
| `burpsuite_alternative_scan` | Comprehensive web scanning to identify race-prone endpoints |
| `browser_agent_inspect` | Browser inspection to observe client-side state changes from parallel requests |

---

## RELATED ROUTING

- **business-logic** — workflow, coupon abuse, and logic-first checklists (`../business-logic/SKILL.md`).
