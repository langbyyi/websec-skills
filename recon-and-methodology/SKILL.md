...
| Signal | Probe | Why |
|---|---|---|
| Scope defined? | List in-scope domains, IPs, ASNs | Boundaries determine what to test |
| Subdomain enum? | `subfinder -d target.com` + `crt.sh` query | Broadens surface — bugs live on forgotten subdomains |
| Port scan done? | `nmap -T4 -F target` or `rustscan` | Non-standard ports (3000, 8443) hide services |
| Tech detected? | `httpx -tech-detect` or `whatweb` | Framework → known CVEs and specific attack vectors |
| Content discovered? | `ffuf -u https://target/FUZZ -w wordlist` | Hidden paths, API docs, admin panels |
| Vuln scan run? | `nuclei -u https://target -t exposures/` | Quick wins: .git, .env, debug endpoints |

```bash
# End-to-end quick recon pipeline
subfinder -d target.com | httpx -title -tech-detect -status-code | nuclei -t exposures/ -t misconfiguration/
```
...
│   ├── Passive (subfinder, crt.sh, amass) → broad surface
│   ├── Active DNS brute (ffuf, massdns) → find hidden subdomains
│   └── Virtual host discovery → find internal apps
├── Port scan (nmap, masscan)
│   ├── Common web ports open (80, 443, 8080)?
│   ├── Non-standard ports (3000, 5000, 8443)?
│   └── Service ports (22, 21, 3306, 6379)?
├── Technology detection (whatweb, httpx)
│   ├── Framework identified → look up known CVEs
│   ├── Spring Boot → check actuator endpoints
│   └── CMS detected → check version-specific vulns
├── Content discovery
│   ├── Directory brute (ffuf, feroxbuster) → hidden paths
│   ├── Parameter discovery (arjun, x8) → hidden params
│   ├── JS mining (gau, LinkFinder) → API endpoints in source
│   └── API docs (swagger, graphql) → full API surface
└── Vulnerability scan (nuclei)
    ├── Run exposure templates → quick wins
    ├── Run misconfiguration templates → config flaws
    └── Prioritize findings by asset value
```

## 10. NUCLEI TEMPLATES (AUTOMATED SCANNING)
...
## 11. COMMON MISCONFIGURATIONS (QUICK WINS)
...
## 13. JAVA MIDDLEWARE FINGERPRINT MATRIX
...
## 14. INFORMATION LEAK DETECTION CHECKLIST
...
- [ ] Define scope (domains, IPs, ASNs, out-of-scope exclusions)
- [ ] Subdomain enumeration (subfinder, amass, crt.sh)
- [ ] Port scanning (nmap, rustscan) on all discovered hosts
- [ ] Technology fingerprinting (httpx, wappalyzer, whatweb)
- [ ] Content and directory discovery (ffuf, feroxbuster, dirsearch)
- [ ] URL and parameter discovery (katana, hakrawler, paramspider, waybackurls)
- [ ] WAF detection (wafw00f)
- [ ] Vulnerability scanning (nuclei with appropriate templates)
- [ ] Information leak detection (.git, .env, backup files, debug endpoints)
- [ ] JavaScript bundle analysis for API endpoints and secrets
...
| Tool | Use Case |
|------|----------|
| `nmap_scan` | Port scanning and service detection on discovered subdomains and IPs |
| `rustscan_fast_scan` | Ultra-fast port discovery for large IP ranges before detailed nmap scans |
| `subfinder_scan` | Passive subdomain enumeration from multiple certificate and DNS sources |
| `httpx_probe` | HTTP probing and technology detection on discovered subdomains |
| `katana_crawl` | Next-generation web crawling and spidering for endpoint discovery |
| `hakrawler_crawl` | Lightweight web endpoint discovery from JS, forms, and robots.txt |
| `waybackurls_discovery` | Historical URL discovery from Wayback Machine for forgotten endpoints |
| `gau_discovery` | URL discovery from Wayback, CommonCrawl, OTX, and URLScan sources |
| `paramspider_discovery` | Parameter mining from web archives to find hidden API parameters |
| `nuclei_scan` | Automated vulnerability scanning with community templates for misconfigurations and exposures |

## RELATED ROUTING

- [api-security](../api-security/SKILL.md) — API surface discovery follows initial recon
- [source-code-exposure](../source-code-exposure/SKILL.md) — source exposure detection builds on recon findings
- [ssrf](../ssrf/SKILL.md) — internal recon and SSRF testing on discovered endpoints
- [burp-mcp](../burp-mcp/SKILL.md) — proxy history analysis for vulnerability verification after recon
