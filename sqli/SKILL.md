...
If you just confirmed a suspected SQL sink, do not load additional payload skills first — complete the first round of verification directly here.
...
```
SQL-like behavior detected in parameter?
├── Visible SQL errors?
│   ├── Error-based extraction → use DBMS-specific error functions
│   └── Fingerprint DBMS from error format → route to DBMS-specific chains
├── UNION keyword reflected or accepted?
│   ├── ORDER BY for column count → UNION SELECT with NULL columns → extract data
│   └── String column found → CONCAT/group_concat for data extraction
├── Boolean response difference (1=1 vs 1=2)?
│   └── Boolean blind → SUBSTRING/ASCII character-by-character extraction
├── No visible output at all?
│   ├── Time delay works (SLEEP/WAITFOR)?
│   │   └── Time-based blind → conditional delays per character
│   └── Time delay fails or unreliable?
│       └── Out-of-band (OOB) → DNS/HTTP/SMB exfiltration via DBMS functions
├── WAF or filter blocking payloads?
│   ├── Comment injection / case variation / URL encoding
│   ├── Whitespace alternatives / string construction
│   └── Polyglot or routed injection
└── Data stored then reused in another query?
    └── Second-order injection → store payload, trigger on reuse
```

## TESTING CHECKLIST

- [ ] Identify all injection points: URL params, POST body, JSON fields, HTTP headers (X-Forwarded-For, User-Agent, Referer, Cookie)
- [ ] Confirm injection with single quote `'` and observe behavioral differences (`'` vs `''`, `1` vs `1-1`)
- [ ] Fingerprint DBMS from error messages or version queries (MySQL, MSSQL, Oracle, PostgreSQL, SQLite)
- [ ] Test error-based extraction using DBMS-specific error functions
- [ ] Test UNION-based extraction: ORDER BY for column count, NULL columns, find string-compatible column
- [ ] Test boolean blind: `AND 1=1` vs `AND 1=2` for conditional response differences
- [ ] Test time-based blind: SLEEP/WAITFOR/DBMS_PIPE per DBMS when no visible output
- [ ] Test out-of-band (OOB) exfiltration: DNS, HTTP, SMB channels for blind or batch scenarios
- [ ] Attempt WAF bypass: comment injection, case variation, URL/double encoding, whitespace alternatives, string construction
- [ ] Test database-specific features: stored procedures, metadata tables, file operations
- [ ] Escalate to OS command execution: xp_cmdshell (MSSQL), UDF (MySQL), Java stored procedures (Oracle)
- [ ] Test second-order injection: store payload, trigger when data is reused in another query
- [ ] Test parameterized query bypass: dynamic table/column names, partial parameterization, IN clause gaps

## MCP TOOLS

| Tool | Use Case |
|------|----------|
| `sqlmap_scan` | Automated SQL injection detection and exploitation against target URLs |
| `http_framework_test` | Craft custom HTTP requests to test manual SQLi payloads |
| `http_repeater` | Replay and modify requests to fine-tune injection attempts |
| `api_fuzzer` | Fuzz API endpoints for SQL injection in parameters and body |

## 17. WAF BYPASS MATRIX
...
- [authentication-bypass](../authentication-bypass/SKILL.md) — SQLi login bypass is a core auth bypass technique
- [nosql-injection](../nosql-injection/SKILL.md) — NoSQL injection shares input-sink patterns with SQLi
- [api-security](../api-security/SKILL.md) — API endpoints are prime SQLi targets via query and body parameters
- [ldap-injection](../ldap-injection/SKILL.md) — LDAP injection is the directory-service analogue of SQLi
