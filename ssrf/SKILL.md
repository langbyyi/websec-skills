...
If you just found a parameter that fetches a URL, do the first round of confirmation directly here.
...
- [ ] Identify URL parameters: `url=`, `path=`, `dest=`, `callback=`, `src=`, `link=`, `file=`, `resource=`
- [ ] Test less obvious vectors: PDF generators, webhooks, RSS imports, OAuth redirects, X-Forwarded-Host headers
- [ ] Confirm SSRF with collaborator/interact.sh callback URL
- [ ] Test internal access: `127.0.0.1`, `localhost`, `[::1]` on common ports (8080, 6379, 9200, 2375, 5984)
- [ ] Test cloud metadata endpoints: AWS IMDSv1/v2 (169.254.169.254), GCP, Azure, Alibaba, Kubernetes
- [ ] Test IP filter bypass: decimal, hex, octal representations; IPv6-mapped IPv4; DNS names resolving to internal IPs
- [ ] Test protocol smuggling: `file://`, `gopher://`, `dict://`, `sftp://`, `ldap://`
- [ ] Test DNS rebinding: initial-legit then second-internal resolution, `nip.io` services
- [ ] Test URL parser confusion: `#@`, `\@`, `%00@`, mixed encoding
- [ ] Test redirect chains: benign external URL 302-redirecting to internal target
- [ ] Test blind SSRF: timing analysis, error message differences, DNS-only callbacks
- [ ] Attempt internal service exploitation: Docker API, Elasticsearch, Redis, Spring Boot actuators

## MCP TOOLS

| Tool | Use Case |
|------|----------|
| `http_framework_test` | Craft custom HTTP requests to test SSRF via URL parameters and headers |
| `http_repeater` | Replay and modify requests to test redirect chains and filter bypasses |
| `burpsuite_alternative_scan` | Comprehensive web scanning to discover SSRF surface automatically |
| `browser_agent_inspect` | Inspect pages for client-side SSRF vectors and JavaScript-based URL fetching |
| `nuclei_scan` | Run SSRF-specific vulnerability templates against target URLs |

## 11. THE SSRF-FILTER MINDSET
...
- [request-smuggling](../request-smuggling/SKILL.md) — HTTP smuggling in proxy chains can enable SSRF
- [command-injection](../command-injection/SKILL.md) — SSRF can chain to command execution via internal services (Redis, Docker API)
- [recon-and-methodology](../recon-and-methodology/SKILL.md) — SSRF enables internal network recon and service enumeration
- [xxe](../xxe/SKILL.md) — XXE is a common SSRF vector via external entity URLs
