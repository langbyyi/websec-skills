...
> **AI LOAD INSTRUCTION**: PHP `==` coercion, magic hashes (`0e…`), HMAC/hash loose checks, NULL from bad types, and CTF-style `strcmp` / `json_decode` / `intval` tricks. Use strict routing: map the sink (`==` vs `hash_equals`), PHP major version, and whether both operands are attacker-controlled. Routing: when encountering PHP login/signature checks or `md5($_GET['x'])==md5($_GET['y'])` style challenges or code, prioritize this skill; if `hash_equals`/`===` is already used, this attack path typically does not apply.
...
| Clue | Next Step |
|---|---|
| Source contains `==` comparing passwords, tokens, or HMAC results | Go to Section 1–3 |
| `md5($a) == md5($b)` or `sha1` loose comparison | Section 2 magic hashes |
| `hash_hmac(...) != '0'` or comparing with `"0"` | Section 3 |
...
- [ ] Fingerprint PHP version from `X-Powered-By` or error messages (PHP 5/7 vs 8 behavior differs significantly)
- [ ] Test loose comparison bypass on login: `password[]=x` (array vs string → NULL collision), `password=0`, `password=true`
- [ ] Test magic hash collisions: `?a=240610708&b=QNKCDZO` for MD5, `?a=10932435112&b=<sha1-magic>` for SHA-1
- [ ] Test numeric coercion: `0 == "admin"` (PHP 7 true, PHP 8 false), `"123a" == 123` (true), `"" == 0` (true)
- [ ] Test HMAC/signature loose compare: brute-force message until `hash_hmac` output matches `^0e[0-9]+$`
- [ ] Test `strcmp` bypass with arrays: `?p[]=1` to force NULL return from `strcmp($_GET['p'], $secret)`
- [ ] Test `json_decode` + `true`: send `{"password":true}` or `{"password":0}` to APIs that decode JSON for auth
- [ ] Test `intval` bypass: `?id=0x1A`, `?id=010`, `?id=1e2` for hex/octal/scientific coercion
- [ ] Test `is_numeric` + loose compare chain: `"0e12345"` passes `is_numeric` and equals `0` under `==`
- [ ] Verify fix: confirm `===`, `hash_equals()`, or strict type declarations are used for secret/token comparisons

## MCP TOOLS

| Tool | Use Case |
|------|----------|
| `http_framework_test` | Send crafted HTTP requests with type juggling payloads (`password[]=x`, `0e12345`, JSON `{"password":true}`) to authentication and token validation endpoints |
| `http_repeater` | Replay and modify type juggling payloads to test magic hash collisions, array-based NULL juggling, and loose comparison bypasses across login/token APIs |
| `nuclei_scan` | Run Nuclei templates for PHP type juggling and weak comparison vulnerabilities to automate detection across multiple endpoints |
| `api_fuzzer` | Fuzz API parameters with type-confusion payloads (arrays, booleans, magic hash strings) to discover endpoints using loose comparison on secrets/tokens |
...
- [authentication-bypass](../authentication-bypass/SKILL.md) — type confusion commonly bypasses authentication checks
- [sqli](../sqli/SKILL.md) — SQL injection with type coercion issues in numeric comparison
- [jwt-attacks](../jwt-attacks/SKILL.md) — JWT algorithm confusion shares type confusion patterns
