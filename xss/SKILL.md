...
- [upload insecure files](../file-upload/SKILL.md) when you need the full upload path: validation, storage, preview, and sharing behavior
...
| Signal | Probe | Why |
|---|---|---|
| Input reflected in response | `<script>alert(1)</script>` | Classic detection — but context matters |
| Reflected but `<script>` filtered | `<svg onload=alert(1)>` | Event handler on SVG bypasses tag blacklist |
| Reflected in HTML attribute | `" autofocus onfocus=alert(1)//` | Break out of attribute, trigger event |
| Reflected in JavaScript string | `'-alert(1)-'` | Break out of string context |
| Reflected in URL / href | `javascript:alert(1)` | Protocol handler sink |
| Filter blocks common payloads | Case mutation, null bytes, fragmented injection | WAF / filter bypass chain |
| CSP header present | JSONP on allow-listed domain, AngularJS CDN, `base-uri` missing | CSP bypass techniques |

```bash
# Quick test — reflected XSS probe
curl -s 'https://target/page?q=<svg onload=alert(1)>' | grep '<svg'
```
...
- [ ] Identify all input vectors: URL params, form fields, HTTP headers, filenames, EXIF metadata, postMessage origins
- [ ] Determine injection context before payload selection: HTML body, attribute value, JavaScript string, URL/href sink, block tag
- [ ] Test reflected XSS with context-appropriate payloads (not just `<script>alert(1)</script>`)
- [ ] Test stored XSS: profile fields, display names, forum posts, comments, admin-visible data
- [ ] Test DOM-based XSS: trace `innerHTML`, `eval`, `document.write`, `location.href` sinks in JavaScript source
- [ ] Test filter bypass: case variation, tag mutation, fragmented injection, null bytes, encoding chains
- [ ] Test CSP bypass: JSONP endpoints on allow-listed domains, AngularJS CDN, base-uri injection, dangling markup
- [ ] Test polyglot payloads across multiple reflection contexts simultaneously
- [ ] Test blind XSS: contact forms, User-Agent, Referer, error log injections with callback payload
- [ ] Test multi-reflection scenarios: double/triple reflection, two-parameter combinations
- [ ] Verify exploit chain: cookie steal, keylogger, CSRF via XSS, post-exploitation pivots

## MCP TOOLS

| Tool | Use Case |
|------|----------|
| `dalfox_xss_scan` | Automated XSS scanning with DOM mining and dictionary-based detection |
| `xsser_scan` | Automated XSS vulnerability testing with multiple injection techniques |
| `http_framework_test` | Craft custom HTTP requests to test manual XSS payloads |
| `http_repeater` | Replay and modify requests for XSS payload tuning |
| `browser_agent_inspect` | Inspect pages for DOM-based XSS and client-side rendering sinks |
| `nuclei_scan` | Run XSS-specific vulnerability templates against target URLs |

## 12. XSS TESTING PROCESS (ZSEANO METHOD)
